...
<section id="bloggrid" class="content row">
    <article role="main"<?php echo !is_active_sidebar( 'blog_sidebar' )?' class="no-sidebar"':''; ?>>
        ...
        <div class="blogpost">
            ...
            <p><?php echo wp_trim_words(get_the_content($post->ID), 50, '<br /><a href="' . get_permalink($post->ID) . '">Read More</a>'); ?></p>
            <div class="post-meta">
                <div class="author"><i class="icon-editor"></i><div><?php the_author(); ?></div></div>
                <div class="comments"><i class="icon-comments"></i><div><?php comments_number( 'No comments', 'One comment', '% comments' ); ?></div></div>
                <div class="cats"><i class="icon-category"></i><div><?php echo cats($post->ID); ?></div></div>
