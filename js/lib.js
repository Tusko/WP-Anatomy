...
//return document
  $.screen = function (type) {
    'use strict';
    if (type === 'height') {
      type = window.innerHeight || document.documentElement.clientHeight;
    } else if (type === 'width') {
      type = document.body.offsetWidth;
    } else {
      type = Math.max(document.documentElement.scrollTop, document.body.scrollTop);
    }
    return method;
  }
