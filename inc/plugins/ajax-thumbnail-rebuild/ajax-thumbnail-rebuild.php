...
class AjaxThumbnailRebuild {
    ...
    function addRebuildSingle($fields, $post)
    {
        ...
        <input type='button' onclick='javascript:regenerate();' class='button' name='ajax_thumbnail_rebuild'
               id='ajax_thumbnail_rebuild' value='Rebuild Thumbnails'>
        <span id="atr-message" class="updated fade"
              style="clear:both;display:none;line-height:28px;padding-left:10px;"></span>
        <?php
        $html = ob_get_clean();
	...
	function ManagementPage()
    {
        ...
        <form method="post" action="" style="display:inline; float:left; padding-right:30px;">
            ...
            <ul id="sizeselect">
                ...
                <?php foreach (ajax_thumbnail_rebuild_get_sizes() as $image_size) : ?>
                    ...
                    <li>
                        ...
                        <label>
                            <input type="checkbox" name="thumbnails[]" id="sizeselect" checked="checked"
                                   value="<?php echo $image_size['name'] ?>"/>
                            <?php
                            $crop_setting = '';
            ...
            <input type="button" onClick="javascript:regenerate();" class="button" name="ajax_thumbnail_rebuild"
                   id="ajax_thumbnail_rebuild" value="<?php _e('Rebuild All Thumbnails', 'ajax-thumbnail-rebuild') ?>"/>
            <br/>
        </form>

        <div id="thumb" style="display:none;"><h4><?php _e('Last image', 'ajax-thumbnail-rebuild'); ?>:</h4><img
                id="thumb-img"/></div>
